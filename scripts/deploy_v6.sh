#!/bin/bash
# KiCryp Trinity v6.0 — Automated Manual Deployment Helper
# Repo: https://github.com/frahmat68-beep/Kibot

INDODAX_IP="213.35.118.26"
BINANCE_IP="152.69.218.198"
REMOTE_USER="ubuntu"
REMOTE_DIR="/home/ubuntu/KiCryp/scripts"
...
FILES=(
    ...
    "scripts/kicryp_whatif_engine.py"
    "scripts/kicryp_timeframe_analyzer.py"
    "scripts/kicryp_learning_engine.py"
)

echo "🚀 Starting KiCryp v6.0 Deployment..."
...
deploy_to_server() {
    ...
    # 2. Remote Restart
    echo "   🔄 Restarting kicryp-engine service..."
    ssh -o ConnectTimeout=5 -o StrictHostKeyChecking=no -o UserKnownHostsFile=/dev/null -i "$key" "$REMOTE_USER@$ip" "sudo systemctl restart kicryp-engine && sudo systemctl status kicryp-engine --no-pager"
