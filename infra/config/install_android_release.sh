...
if [[ ! -f "${APK_PATH}" ]]; then
  echo "Release APK not found. Run infra/config/build_android_release.sh first."
  exit 1
fi

if ! command -v adb >/dev/null 2>&1; then
  echo "adb is not available in PATH. Run infra/config/setup_android_sdk.sh first."
  exit 1
fi
