...
for rel in ("core", "tools", "scanners"):
    sys.path.insert(0, str(ROOT_DIR / rel))
...
def test_brain():
    ...
    with (
        ...
        print(f"World model: {snapshot.get('world_model')}")
        print(f"Strategy next: {snapshot.get('daily_target', {}).get('strategy_next')}")
        if isinstance(snapshot.get("world_model"), dict) and snapshot["world_model"].get("source_status"):
            print("✅ World model assembled.")
        else:
            print("❌ World model missing.")
