...
echo -n "[CORE] kibot_manager.py... "
if [ -f "core/kibot_manager.py" ]; then
    echo -e "${GREEN}PASSED${NC}"
else
    echo -e "${RED}FAILED (Missing/Not Renamed)${NC}"
fi

echo -n "[CORE] kibot_local_signal.py... "
if [ -f "core/kibot_local_signal.py" ]; then
    echo -e "${GREEN}PASSED${NC}"
else
    echo -e "${RED}FAILED (Missing)${NC}"
fi
...
if grep -q "run_local_signal_engine_manager" core/kibot_manager.py 2>/dev/null; then
    echo -e "${GREEN}PASSED${NC}"
else
...
if [ -n "${SUPABASE_URL:-}" ] || grep -q "SUPABASE_URL=" "${PROJECT_ROOT}/.env" 2>/dev/null || grep -q "SUPABASE_URL=" "${PROJECT_ROOT}/.env.kibot" 2>/dev/null; then
    echo -e "${GREEN}PASSED${NC}"
else
    echo -e "${RED}FAILED (Missing)${NC}"
fi
