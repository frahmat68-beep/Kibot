...
explore = PairStats(pair="explore", trade_count=4, win_count=2, loss_count=2, sum_wins=0.03, sum_losses=0.028, ema_pnl=-0.001, alpha=3, beta=3)
explore_allowed, explore_reason = explore.should_entry(
    {
...
check("world model override allows controlled exploration", explore_allowed, explore_reason)
...
with (
    ...
    patch.object(brain, "_get_x_market_brief", return_value={}),
    patch.object(brain, "_get_gdelt_market_brief", return_value={}),
    patch.object(brain, "_get_coingecko_trending", return_value={"coins": []}),
    ...
    check("brain world model available", isinstance(snapshot.get("world_model"), dict) and bool(snapshot.get("world_model", {}).get("source_status")), str(snapshot.get("world_model")))
