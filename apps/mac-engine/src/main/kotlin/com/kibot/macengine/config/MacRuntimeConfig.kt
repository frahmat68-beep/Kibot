...
data class MacRuntimeConfig(
    ...
    val dailyRiskRefreshIntervalMillis: Long = 15_000L,
...
object MacRuntimeConfigLoader {
    fun load(cwd: Path = Paths.get("").toAbsolutePath()): MacRuntimeConfig {
        ...
        return MacRuntimeConfig(
            ...
            pollIntervalMillis = optional("BOT_POLL_INTERVAL_MS")?.toLongOrNull() ?: 1_500L,
            exchangePingRefreshIntervalMillis = optional("BOT_EXCHANGE_PING_REFRESH_INTERVAL_MS")?.toLongOrNull() ?: 1_800L,
            balanceRefreshIntervalMillis = optional("BOT_BALANCE_REFRESH_INTERVAL_MS")?.toLongOrNull() ?: 2_200L,
            openOrdersRefreshIntervalMillis = optional("BOT_OPEN_ORDERS_REFRESH_INTERVAL_MS")?.toLongOrNull() ?: 2_200L,
            dailyRiskRefreshIntervalMillis = optional("BOT_DAILY_RISK_REFRESH_INTERVAL_MS")?.toLongOrNull() ?: 15_000L,
            devicesRefreshIntervalMillis = optional("BOT_DEVICES_REFRESH_INTERVAL_MS")?.toLongOrNull() ?: 30_000L,
            commandsRefreshIntervalMillis = optional("BOT_COMMANDS_REFRESH_INTERVAL_MS")?.toLongOrNull() ?: 1_800L,
            weeklySummaryRefreshIntervalMillis = optional("BOT_WEEKLY_SUMMARY_REFRESH_INTERVAL_MS")?.toLongOrNull() ?: 60_000L,
            recentOrdersRefreshIntervalMillis = optional("BOT_RECENT_ORDERS_REFRESH_INTERVAL_MS")?.toLongOrNull() ?: 2_800L,
            recentFillsRefreshIntervalMillis = optional("BOT_RECENT_FILLS_REFRESH_INTERVAL_MS")?.toLongOrNull() ?: 2_800L,
