...
INDODAX_KEY="${INDODAX_KEY:-${ROOT_DIR}/SG-Executor/SSH/SSH_EXECUTOR/ssh-key-2026-03-22.key}"
BINANCE_KEY="${BINANCE_KEY:-${ROOT_DIR}/SG-Scanner/SSH_SCANNER/ssh-key-2026-03-27.key}"
...
safe_ssh_section "KiBot" "$INDODAX_HOST" "$INDODAX_KEY" "
set -e
printf 'host=%s\n' '$INDODAX_HOST'
printf 'service=%s\n' \"\$(systemctl is-active kibot-executor-indodax)\"
...
printf 'warn_cp_12s=%s\n' \"\$(journalctl -u kibot-executor-indodax --since '$LOG_WINDOW' --no-pager | grep -c 'Timed out waiting for 12000 ms' || true)\"
printf 'warn_oom=%s\n' \"\$(journalctl -u kibot-executor-indodax --since '$LOG_WINDOW' --no-pager | grep -c 'Failed to parse OOM policy' || true)\"
printf 'warn_health_500=%s\n' \"\$(journalctl -u kibot-executor-indodax --since '$LOG_WINDOW' --no-pager | grep -c '500 Internal Server Error: GET - /api/health' || true)\"
...
safe_ssh_section "KiBot" "$BINANCE_HOST" "$BINANCE_KEY" "
set -e
printf 'host=%s\n' '$BINANCE_HOST'
...
printf 'jar_sha=%s\n' \"\$(sha256sum /home/ubuntu/KiBot/server/mac-engine-all.jar | cut -d' ' -f1)\"
printf 'warn_cp_12s=%s\n' \"\$(journalctl -u kibot-executor-indodax --since '$LOG_WINDOW' --no-pager | grep -c 'Timed out waiting for 12000 ms' || true)\"
printf 'warn_stopped=%s\n' \"\$(journalctl -u kibot-executor-indodax --since '$LOG_WINDOW' --no-pager | grep -c 'LIFECYCLE_BLOCK: Cannot start sync cycle' || true)\"
printf 'warn_json=%s\n' \"\$(journalctl -u kibot-executor-indodax --since '$LOG_WINDOW' --no-pager | grep -c 'Ignoring malformed dashboard JSON file' || true)\"
printf 'warn_health_500=%s\n' \"\$(journalctl -u kibot-executor-indodax --since '$LOG_WINDOW' --no-pager | grep -c '500 Internal Server Error: GET - /api/health' || true)\"
printf 'legacy_kibot_recovery=%s/%s\n' \"\$(systemctl is-enabled kibot-recovery.service 2>/dev/null || echo not-found)\" \"\$(systemctl is-active kibot-recovery.service 2>/dev/null || echo inactive)\"
...
printf 'legacy_kibot_engine_recovery_timer=%s/%s\n' \"\$(systemctl is-enabled kibot-engine-recovery.timer 2>/dev/null || echo not-found)\" \"\$(systemctl is-active kibot-engine-recovery.timer 2>/dev/null || echo inactive)\"
printf 'legacy_KiBot_engine_recovery_timer=%s/%s\n' \"\$(systemctl is-enabled kibot-executor-indodax-recovery.timer 2>/dev/null || echo not-found)\" \"\$(systemctl is-active kibot-executor-indodax-recovery.timer 2>/dev/null || echo inactive)\"
"
