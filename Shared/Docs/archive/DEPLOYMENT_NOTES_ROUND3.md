...
Logika mustahil menyebabkan **false disconnect** — bot menganggap KiBot/KiCryp Manager mati padahal masih aktif.
...
# Restart services
sudo systemctl restart kibot-executor-indodax
sudo systemctl restart kicryp-manager

# Verify services running
sudo systemctl status kibot-executor-indodax
sudo systemctl status kicryp-manager

# Watch logs for new logic
tail -f /var/log/kicryp/kibot-executor-indodax.log | grep "BARBARIAN_DECAY_EXIT"
```
...
tail -f /var/log/kicryp/kibot-executor-indodax.log | grep -i "trinity.*disconnect\|heartbeat.*timeout"

# Expected: NO false timeouts
# If KiBot truly down, timeout will still trigger after 300ms (correct)
```
...
# Restore backup JAR
cd ~/kicryp
sudo systemctl stop kibot-executor-indodax kicryp-manager
...
sudo systemctl start kibot-executor-indodax kicryp-manager
