...
- Do not split `KiBot`, `KiBot`, and `KiCryp` into separate long-lived branches.
...
## Deployment Split
- `deploy-KiBot.yml` -> KiBot / Indodax server only
- `deploy-KiBot.yml` -> KiBot / Binance server only
...
- If a change is Binance-only, keep KiBot deploy path ignored unless intentional.
