...
- jangan kurangi jatah `kibot-executor-indodax` / `kibot-executor-indodax` tanpa alasan kuat
- AI berat dan review nightly harus pindah ke Batam

## Smoke Test

1. `systemctl is-active` semua service inti
2. `curl /api/state` EXECUTOR dan SCANNER normal
...
- `journalctl -u kibot-executor-indodax --since "10 min ago"`
- `journalctl -u kibot-executor-indodax --since "10 min ago"`
