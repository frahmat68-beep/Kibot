...
- Node A (primary): `213.35.118.26` / `kibot-executor-indodax`
- Node B (radar): `152.69.218.198` / `kibot-executor-indodax`
- APIs:
...
```bash
systemctl is-active kibot-executor-indodax kibot-manager
...
```bash
systemctl is-active kibot-executor-indodax kibot-manager kibot-notifier kibot-guardian kibot-auditor kibot-orchestrator kibot-security
...
```bash
systemctl is-active kibot-recovery kibot-local-scanner kibot-coordinator kibot-engine-recovery.timer kibot-executor-indodax-recovery.timer || true
```
...
```bash
journalctl -u kibot-executor-indodax --since "15 min ago" --no-pager | egrep -i "DEAD_ZONE_TRACE|EXCHANGE_FETCH|health warnings detail|FAILED|Invalid symbol|stale|duplicate|already exists"
journalctl -u kibot-executor-indodax --since "15 min ago" --no-pager | egrep -i "DEAD_ZONE_TRACE|EXCHANGE_FETCH|health warnings detail|FAILED|Invalid symbol|stale|duplicate|already exists"
...
3. Restart only affected engine:
   - `sudo systemctl restart kibot-executor-indodax` or `sudo systemctl restart kibot-executor-indodax`
...
1. `kibot-executor-indodax` (Node B)
2. `kibot-manager` (both nodes if required)
3. `kibot-executor-indodax` (Node A)
