...
class PreferencesManager(context: Context) {
    ...
    companion object {
        ...
        private const val LEGACY_DEFAULT_HOST = "213.35.118.26"
    }

    fun getServerConfig(): ServerConfig {
        val storedHost = prefs.getString(KEY_SERVER_HOST, null)?.trim().orEmpty()
        val host = when {
            storedHost.isBlank() -> ServerConfig.DEFAULT_HOST
            storedHost == LEGACY_DEFAULT_HOST -> ServerConfig.DEFAULT_HOST
            else -> storedHost
        }
        if (host != storedHost) {
            prefs.edit().putString(KEY_SERVER_HOST, host).apply()
        }
        val port = prefs.getInt(KEY_SERVER_PORT, ServerConfig.DEFAULT_PORT)
