...
def test_capital_scenarios() -> None:
    ...
    def bucket_a_can_enter(KiBot_signal: dict, kicom_signal: dict, signal_ttl_s: int = 5) -> bool:
        return KiBot_signal.get("age_s", 999) < signal_ttl_s and kicom_signal.get("age_s", 999) < signal_ttl_s

    test("C21: Both fresh => enter", bucket_a_can_enter({"age_s": 2}, {"age_s": 3}))
    test("C22: KiBot stale => block", not bucket_a_can_enter({"age_s": 6}, {"age_s": 2}))
...
def test_udp_scenarios() -> None:
    ...
    ports = {"kibot-manager": 9998, "kibot-executor-indodax": 8787, "kibot-executor-indodax": 8788, "kibot-manager-udp": 9999}
